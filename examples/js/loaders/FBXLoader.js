...
export default class FBXLoader extends THREE.Loader {
	...
	parse( text ) {
		...
		var nodes = new FBXLoader.FBXParser().parse( text );
		console.timeEnd( 'FBXLoader: TextParser' );

		console.time( 'FBXLoader: ObjectParser' );
		scope.hierarchy = ( new FBXLoader.Bones() ).parseHierarchy( nodes );
		scope.weights	= ( new FBXLoader.Weights() ).parse( nodes, scope.hierarchy );
		scope.animations = ( new FBXLoader.Animation() ).parse( nodes, scope.hierarchy );
		scope.textures = ( new FBXLoader.Textures() ).parse( nodes, scope.hierarchy );
		...
		if ( geoCount > 0 ) {
			for ( geo in node.Objects.subNodes.Geometry ) {
				...
				if ( node.Objects.subNodes.Geometry[ geo ].attrType === 'Mesh' ) {
					// Need to get modelId so we can get the name of the mesh from export
					var modelId = this.getModelIdForGeometryId(node.Connections.properties.connections, geo);
					res.push(
						this.parseGeometry(
							node.Objects.subNodes.Geometry[ geo ],
							node,
							node.Objects.subNodes.Model[modelId].attrName
						)
					);
	...
	getModelIdForGeometryId(connections, geo) {
		var modelId = undefined;
		connections.some(function(connection){
			// [0] = geom, [1] = model
			if(connection[0] == geo) {
				modelId = connection[1];
				return true;
			}
		});
	...
	// name is optional
	parseGeometry( node, nodes, name ) {
		var geo = ( new FBXLoader.Geometry() ).parse( node );
		...
		// add name from model to mesh properties
		if(name)
			mesh.name = name;
	...
	addAnimation( mesh, matrices, animations ) {
	...
	static parse_Data_ByPolygonVertex_IndexToDirect( node, indices, itemSize ) {
	...
	static parse_Data_ByPolygonVertex_Direct( node, indices, strides, itemSize ) {
	...
	static mapByPolygonVertexToByVertex( data, indices, stride ) {
	...
	static toRad( arr ) {
		...
		return arr.map( function ( element ) {
			...
			return FBXLoader.degToRad( element );
...
FBXLoader.FBXParser = class {
	...
	parse( text ) {
		...
		this.allNodes = new FBXLoader.FBXNodes();
		this.nodeStack = [];
		this.currentProp = [];
...
FBXLoader.Bones = class {
	...
	parseHierarchy(node) {
		...
		for (var i = 0; i < bones.length; ++i) {
			...
			if ('Lcl_Translation' in bone.properties) {
				...
				t = FBXLoader.toFloat(bone.properties.Lcl_Translation.value.split(','));

			}

			if ('Lcl_Rotation' in bone.properties) {

				r = FBXLoader.toRad(FBXLoader.toFloat(bone.properties.Lcl_Rotation.value.split(',')));
			...
			if ('Lcl_Scaling' in bone.properties) {
				...
				s = FBXLoader.toFloat(bone.properties.Lcl_Scaling.value.split(','));
	...
	getBoneIdfromInternalId(node, id) {
...
FBXLoader.Geometry = class {
	...
	parse(geoNode) {
		...
		this.uvs = ( new FBXLoader.UV() ).parse(this.node, this);
		this.normals = ( new FBXLoader.Normal() ).parse(this.node, this);
	...
	convertPolyIndicesToTri(indices, strides) {
...
FBXLoader.UV = class {
	...
	_parseText(node) {
		...
		this.uv = FBXLoader.toFloat(uvs.split(','));
		this.index = FBXLoader.toInt(uvIndex.split(','));
	...
	parse(node, geo) {
		...
		switch (mappingType) {
			...
			case "ByPolygonVertex":
				...
				// convert from by polygon(vert) data into by verts data
				this.uv = FBXLoader.mapByPolygonVertexToByVertex(this.uv, geo.getPolygonVertexIndices(node), 2);
				break;
	...
	parseUV_ByPolygonVertex_Direct(node, indices, strides, itemSize) {

		return FBXLoader.parse_Data_ByPolygonVertex_Direct(node, indices, strides, itemSize);

	}

	parseUV_ByPolygonVertex_IndexToDirect(node, indices) {

		return FBXLoader.parse_Data_ByPolygonVertex_IndexToDirect(node, indices, 2);

	}

	parseUV_ByPolygon_Direct(node) {
	...
	parseUV_ByPolygon_IndexToDirect(node) {
...
FBXLoader.Normal = class {
	...
	_parseText(node) {
		...
		this.normal = FBXLoader.toFloat(rawTextNormals.split(','));
	...
	parseNormal_ByPolygonVertex_Direct(node, indices, strides, itemSize) {

		return FBXLoader.parse_Data_ByPolygonVertex_Direct(node, indices, strides, itemSize);

	}

	parseNormal_ByPolygonVertex_IndexToDirect(node) {
	...
	parseNormal_ByPolygon_Direct(node) {
	...
	parseNormal_ByPolygon_IndexToDirect(node) {
	...
	parseNormal_ByVertex_Direct(node) {
...
FBXLoader.AnimationCurve = class {
	constructor() {
		this.version = null;
